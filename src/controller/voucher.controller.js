...
const voucherModel = require(path.join(
  __dirname,
  "..",
  "models",
  "voucher.model"
    ...
    try {
      voucherModel
        ...
        .then((vouchers) => {
          res.json({
            success: true,
            message: "all vouchers data are retrieved",
            data: vouchers,
          });
        })
        .catch((error) =>
      ...
      voucherModel
        ...
        .then((voucher) => {
          if (voucher) {
            res.json({
              success: true,
              message: "Getting voucher data succefully",
              data: voucher,
            });
          } else
            res
              .status(404)
              .json({ success: false, message: "voucher doesn't exist" });
        })
        .catch((error) =>
          res.status(500).json({ success: false, message: error.message })
        );
